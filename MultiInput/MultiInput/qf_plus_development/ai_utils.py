...
def svd_orthonormal(w, use_existing=False):
...
def LSUVinit(model, data, needed_std = 1.0, std_tol = 0.01, max_attempts = 10, do_orthonorm = True, needed_mean = 0., cuda = True, verbose = False):
    #cuda = data.is_cuda
    gg['total_fc_conv_layers']=0
    gg['done_counter']= 0
    gg['hook_position'] = 0
    ...
    with torch.no_grad():
        ...
        for layer_idx in range(gg['total_fc_conv_layers']):
            if verbose: print (f'layer_idx: {layer_idx}')
